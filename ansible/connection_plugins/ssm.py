...
def _ssm_retry(func):
    ...
    * retries limit reached
    ...
    def wrapped(self, *args, **kwargs):
        ...
        cmd_summary = "%s..." % (len(args) and args[0] or 'connect')
        for attempt in range(remaining_tries):

            try:
                return_tuple = func(self, *args, **kwargs)
                display.vvv(return_tuple, host=self._host)
                break
                ...
                if attempt == remaining_tries - 1:
                    ...
                    display.vv(msg, host=self._host)
...
class Connection(ConnectionBase):
    ...
    def _connect(self):
        ...
        self._websocket = SSMSocket(self._host, self._screen.columns, self._screen.lines)
...
class SSMSocket(websocket.WebSocketApp):
    ...
    def connect(self, timeout=None):
        ...
        self._send_auth()
        self.recv_ssm('output_stream_data')  # wait for initial prompt back
    ...
    def _send_ssm(self, data_utf8, message_type, message_id, seq, is_sequential):
        print("SEND %s (%s)\n%s" % (message_type, message_id, data_utf8))
    ...
    def recv(self, data):
        ...
        print("RECV %s (%s)\n%s" % (payload_header[1].decode('utf8'), recv_message_id, data[payload_header[0]+4:]))
